...
17. **Conditional Importance Test (CIT) outputs:** see `Sim-CIT/CPI/*_results.csv`, `Sim-CIT/RPT/*_results.csv`, and diagnostics in `Sim-CIT/CPI/Diagnostics/*.csv`.
18. **CIT plotting outputs:** `python plot_CIT.py` writes the combined two-panel figure to `Sim-CIT/CPI/Plots/*_feature_cumulative_CIT_estimates_combined_panel*_pos_only_.png` (group-colour panel on the left, significance panel on the right).
...
5. `Sim-CIT/` for conditional importance test inputs/outputs, diagnostics, and combined CIT figure panels generated by `plot_CIT.py`.
